...
class ReceiveThread(threading.Thread):
    ...
    def display(self, flag, str_data, sou=0):
        if flag == -3:
            window.textBrowser.append('From ' + str(sou) + ' received:' + str_data + ' ///Parity Error///')
        elif flag == -2:
            window.textBrowser.append(str_data + ' ///Address Blurred///')
        elif flag == 0:
            window.textBrowser.append(str_data + ' ///Wrong Destination///')
        elif flag == 1:
            window.textBrowser.append('From ' + str(sou) + ' received:' + str_data + ' ///Gotcha!///')
        else:
            return
                ...
                if des == SelfID or des == 7:
                    if data_type == 8:  # 询问地址
                        ...
                        if not isParity:  # 判断奇偶校验结果
                            Error += 1
                            window.num_failed.display(Error)
                            self.display(-3, bit_str, sou)
                            self.ACK(0, sou)
                            continue
                        else:
                            separated.pop(0)
                            byte_str = function.parity_binString_2_decStr(separated)
                            self.display(1, byte_str, sou)
                            self.ACK(1, sou)
                else:  # 本机非目的地址
